...
function Temperature({weather:{
  details, icon, temp, temp_max, temp_min, sunset, sunrise, humidity, speed, feels_like, timezone
}}) {
  return (
    <div>
      ...
      <div className="flex flex-row items-center justify-center space-x-2 text-sm text-white py-3">
        ...
        <UilSunset />
        <p className="font-light">
          Low:<span className="font-medium ml-1 ">{formatToLocalTime(sunset, timezone, "hh:mm a")}</span>
        </p>
        <p className="font-light">|</p>
        <UilSun />
        <p className="font-light">
          High:<span className="font-medium ml-1 ">{`${temp_max
            .toFixed()}`}&deg;</span>
        </p>
        <p className="font-light">|</p>
        <UilSun />
